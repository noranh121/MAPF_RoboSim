...
def setup_A_star():
    ...
    """
    from a_star_tb3.a_star_tb3_script import A_star
    return A_star()
...
def test_is_point_in_any_block_success(setup_A_star):
    ...
    """
    a_star = setup_A_star
    obstacle_space = a_star.convert_map_to_obstacles('bb.txt')
    result=a_star.is_point_in_any_block(1.0,1.0,obstacle_space)
...
def test_is_point_in_any_block_failure(setup_A_star):
    ...
    a_star = setup_A_star
    obstacle_space = a_star.convert_map_to_obstacles('bb.txt')
    result=a_star.is_point_in_any_block(0.0,1.0,obstacle_space)
...
def test_is_point_within_block_success(setup_A_star):
    ...
    result=a_star.is_point_within_block(x_center,y_center,1.0,1.0,size_x/2,size_y/2)
    assert not result, f"Expected {False}, but got {True}"
...
def test_is_point_within_block_failure(setup_A_star):
    ...
    obstacle_space = a_star.convert_map_to_obstacles('bb.txt')
    x_center, y_center, size_x , size_y = obstacle_space[0]
    result=a_star.is_point_within_block(x_center,y_center,0.0,0.0,size_x/2,size_y/2)
    assert result, f"Expected {True}, but got {False}"
